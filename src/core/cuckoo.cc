...
bool CuckooFilter::AddNewSubFilter() {
  const uint64_t bucket_count = SubFilterBucketCount(num_buckets_, expansion_, filters_.size());
  const uint64_t num_slots = bucket_count * slots_per_bucket_;
  SubFilter sf(num_slots, uint8_t{0}, mr_);
